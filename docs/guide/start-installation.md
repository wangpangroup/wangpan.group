...
With Composer installed, you can then install the application using the following commands:
    ...
    composer create-project --prefer-dist wangpangroup/wangpan.group wangpan.group

The command installs the advanced application in a directory named `wangpan.group`. You can choose a different
...
Extract the archive file downloaded from [dev.wangpan.group](http://dev.wangpan.group/download/) to
...
1. Open a console terminal, execute the `init` command and select `dev` as environment.
   ...
   ```
   /path/to/php-bin/php /path/to/wangpan.group/init
   ```

   If you automate it with a script you can execute `init` in non-interactive mode.

   ```
   /path/to/php-bin/php /path/to/wangpan.group/init --env=Production --overwrite=All
   ```
